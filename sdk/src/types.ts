export interface Tip { height: number; hash: string }
...
export interface Block extends BlockSummary {
  ...
  coinbase?: {
    ...
    fee_sats: number;
    total_sats: number;
    maturity: { matured: boolean; confirmations: number; needed: number; blocks_remaining: number };
  };
}
...
export interface TxInput {
  ...
  txinwitness?: string[];
  coinbase?: string;
  prevout?: { value_sats: number; address?: string; script_type?: string };
}
...
export interface AddressStats {
  address: string;
  valid: boolean;
  balance: { confirmed_sats: number; unconfirmed_sats: number };
  tx_count: number;
  label?: string;
  is_special: boolean;
...
export interface MempoolSummary {
  ...
  fee_rate_max: number;
  fee_categories: { low: number; medium: number; high: number };
...
export interface VisionEvent { type: EventType; data: any }
